...
func ListAll(c echo.Context) error {
	...
	defer lock.Unlock()
	t := &dao.Todo{}
	db := c.Get("R_DB").(*mongo.Collection)
...
func ListByCategory(c echo.Context) error {
	lock.Lock()
	defer lock.Unlock()
	t := &dao.Todo{Category: strings.ToLower(c.Param("category"))}
	db := c.Get("R_DB").(*mongo.Collection)
	todos, err := t.ListByCategory(db)
	if err != nil {
		return err
	}
	return c.JSON(http.StatusOK, todos)
